 ...
 */
 export function doesntMatchPasswords(oldPassword,newPassword,alertNoMatchMessage){
    ...
    cy.get(SETTINGS_VARIABLES.CONFIRM_NEWPASS_BUTTON).click({multiple:true , force:true})
    cy.wait(3000)
    cy.get(SETTINGS_VARIABLES.ALERT_NOMATCH_PASSWORD).contains(alertNoMatchMessage)
    cy.get(SETTINGS_VARIABLES.CANCEL_PASSWORD).click({multiple:true , force:true})
}
/**
 * This function test that new password and confirm new password are identical either alert with specific message 
 * @param {string}           oldPassword password that would be cahnged.
 ...
 */
 export function EmptyPasswordAndNoMatch(oldPassword,newPassword,alertNoMatchMessage,alertEmptyMessage){
        ...
        cy.get(SETTINGS_VARIABLES.NEW_PASSWORD_INPUT).clear().type(newPassword)
        cy.get(SETTINGS_VARIABLES.CONFIRM_PASSWORD_INPUT).clear().type(oldPassword)
        cy.get(SETTINGS_VARIABLES.CONFIRM_NEWPASS_BUTTON).click({multiple:true , force:true})
        cy.wait(3000)
 ...
 * @param {string}           alertEmptyMessage alert message that should appear when old password input left empty.
 */
 export function EmptyNewPassword(oldPassword,newPassword,alertEmptyMessage){
    cy.get(SETTINGS_VARIABLES.CHANGE_PASSWORD).click({multiple:true})
    cy.wait(3000)
    cy.get(SETTINGS_VARIABLES.CURRENT_PASS_INPUT).clear().type(oldPassword)
    cy.get(SETTINGS_VARIABLES.NEW_PASSWORD_INPUT).clear()
    cy.get(SETTINGS_VARIABLES.CONFIRM_PASSWORD_INPUT).clear().type(newPassword)
    cy.get(SETTINGS_VARIABLES.CONFIRM_NEWPASS_BUTTON).click({multiple:true , force:true})
    cy.wait(3000)
    cy.get(SETTINGS_VARIABLES.ALERT_EMPTYNEWPASSWORD_PASSWORD).contains(alertEmptyMessage)
    cy.get(SETTINGS_VARIABLES.CANCEL_PASSWORD).click({multiple:true , force:true})
}
/**
 ...
 */
 export function samePassword(oldPassword,alertSamePasswordMessage){
    ...
    cy.wait(3000)
    cy.get(SETTINGS_VARIABLES.CURRENT_PASS_INPUT).clear().type(oldPassword)
    cy.get(SETTINGS_VARIABLES.NEW_PASSWORD_INPUT).clear().type(oldPassword)
    cy.get(SETTINGS_VARIABLES.CONFIRM_PASSWORD_INPUT).clear().type(oldPassword)
    cy.get(SETTINGS_VARIABLES.CONFIRM_NEWPASS_BUTTON).click({multiple:true , force:true})
    cy.wait(5000)
 ...
 */
 export function easyGuessed(oldPassword,easyPassword,alertEasyGuessPasswordMessage){
    ...
    cy.get(SETTINGS_VARIABLES.CONFIRM_PASSWORD_INPUT).clear().type(easyPassword)
    cy.get(SETTINGS_VARIABLES.CONFIRM_NEWPASS_BUTTON).click({multiple:true , force:true})
    cy.wait(5000)
 ...
 */
 export function weakPassword(oldPassword,weakPassword,alertWeakPasswordMessage){
    ...
    cy.wait(3000)
    cy.get(SETTINGS_VARIABLES.CURRENT_PASS_INPUT).clear().type(oldPassword)
    cy.get(SETTINGS_VARIABLES.NEW_PASSWORD_INPUT).clear().type(weakPassword)
    cy.get(SETTINGS_VARIABLES.CONFIRM_PASSWORD_INPUT).clear().type(weakPassword)
    cy.get(SETTINGS_VARIABLES.CONFIRM_NEWPASS_BUTTON).click({multiple:true , force:true})
    cy.wait(5000)
    cy.get(SETTINGS_VARIABLES.ALERT_WEAK_PASSWORD).contains(alertWeakPasswordMessage)
    cy.get(SETTINGS_VARIABLES.CANCEL_PASSWORD).click({multiple:true , force:true})
}
