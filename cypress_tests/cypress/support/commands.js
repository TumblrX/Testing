import { VARIABLES } from '../integration/sign_up_mapping_var';
 ...
 */
Cypress.Commands.add('invalidEmailSignUp', function (emailOption, msg) {
    cy.get(VARIABLES.BEGIN_SIGN_UP).contains('Sign up').click({ force: true })
    if (emailOption != "") {
 ...
 * @param {string}           msg          massage that expected to appeare.
 */
Cypress.Commands.add('invalidPasswordSignUp', function (passOption, msg) {
    cy.get(VARIABLES.BEGIN_SIGN_UP).contains('Sign up').click({ force: true })
    cy.get(VARIABLES.INPUT_EMAIL).type("nadaelsayed163@gmail.com").should("be.visible")
    if (passOption != "") {
 ...
 * @param {string}           msg              massage that expected to appeare.
 */
Cypress.Commands.add('invalidBlogNameSignUp', function (blogNameOption, msg) {
    cy.get(VARIABLES.BEGIN_SIGN_UP).contains('Sign up').click({ force: true })
    cy.get(VARIABLES.INPUT_EMAIL).type("nadaelsayed163@gmail.com").should("be.visible")
    cy.get(VARIABLES.INPUT_PASSWORD).click({ force: true }).type("nadaelsayed147258369").should("be.visible")
 ...
 * @param {string}           msg          massage that expected to appeare.
 */
Cypress.Commands.add('invalidAgeSignUp', function (age_Option, msg) {
    cy.get(VARIABLES.BEGIN_SIGN_UP).contains('Sign up').click({ force: true })
    cy.get(VARIABLES.INPUT_EMAIL).type("nadaelsayed163@gmail.com").should("be.visible")
    cy.get(VARIABLES.INPUT_PASSWORD).click({ force: true }).type("nadaelsayed147258369").should("be.visible")
